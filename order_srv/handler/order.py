...
class OrderServicer(order_pb2_grpc.OrderServicer):
    ...
    def CreateCartItem(self, request: order_pb2.CartItemRequest, context):
        ...
        existed_items = ShoppingCart.select().where(
            ShoppingCart.goods == request.goodsId, ShoppingCart.user == request.userId
        )
        # 如果记录已经存在则合并购物车
        if existed_items:
    ...
    def OrderDetail(self, request: order_pb2.OrderRequest, context):
        ...
        try:
            ...
            for order_good in order_goods:
                order_good_rsp = order_pb2.OrderItemResponse()
                order_good_rsp.goodsId = order_good.goods
                order_good_rsp.goodsName = order_good.goods_name
                order_good_rsp.goodsImage = order_good.goods_image
                order_good_rsp.goodsPrice = float(order_good.goods_price)
                order_good_rsp.nums = order_good.nums
                rsp.data.append(order_good_rsp)

            return rsp
        except DoesNotExist as e:
            context.set_code(grpc.StatusCode.NOT_FOUND)
            context.set_details("订单详情不存在")
            return rsp
    ...
    def local_execute(self, msg, user_args):
        ...
        parent_span = msg_body["parent_span"]
        ...
        with settings.DB.atomic() as txn:
            ...
            with tracer.start_span(
                "select_shopcart", child_of=parent_span
            ) as select_shopcart_span:
                # 获取所有购物车记录
            ...
            # 查询商品的信息
            with tracer.start_span(
                "query_goods", child_of=parent_span
            ) as query_goods_span:
            ...
            with tracer.start_span("query_inv", child_of=parent_span) as query_inv_span:
                inv_srv_host, inv_srv_port = register.get_host_port(
                ...
                try:
                    inv_stub.Sell(
                        inventory_pb2.SellInfo(
                            order_sn=order_sn, goodsInfo=goods_sell_info
                        )
                    )
                except grpc.RpcError as e:
                    local_execute_dict[order_sn]["code"] = grpc.StatusCode.INTERNAL
                    ...
                    err_code = e.code()
                    if (
                        err_code == grpc.StatusCode.UNKNOWN
                        or err_code == grpc.StatusCode.DEADLINE_EXCEEDED
                    ):
                        # 库存不足，库存服务没有扣减库存，不需要发送归还库存信息
                        return TransactionStatus.COMMIT
                    else:
                        return TransactionStatus.ROLLBACK

                # 创建订单
                # 原本比较简单的逻辑应该是：1、本地开始half消息-扣减库存2、执行本地事务3、确定应该确认消息还是回滚消息
                # 1基于可靠消息的最终一致性，只确保自己发送出去的消息是可靠的，不能确保消费者能正确的执行
                # 2、积分服务-这里有一个隐含的点：你的消费者必要保证能成功
                # 3、但是库存服务比较特殊-库存是有限的如果本地事务执行失败应该调用规划库存-TCC:1、并发没有那么高 2、很复杂
                with tracer.start_span(
                    "insert_order", child_of=parent_span
                ) as insert_order_span:
                    try:
                        order = OrderInfo()
                        ...
                        msg.set_delay_time_level(5)  # 设置为超时1min
                        msg.set_keys("zhao")
                        msg.set_tags("cancel")
                        msg.set_body(json.dumps({"orderSn": order_sn}))
                        ...
                        事务消息：
                                1、在扣减库存之前先准备好以后half消息
                                2、如果库存扣减失败：
                                    确认消息 -库存服务就可以确定归还库存
                                3、如果库存扣减成功：
                                    取消消息-
                                    但是返回的时候网络出现抖动 导致超时机制认为这个调用失败
                                        确认消息 -库存服务可以确定归还库存
    ...
    def CreateOrder(self, request: order_pb2.OrderRequest, context):
        ...
        # 要先准备好一个half
        producer = TransactionMQProducer("zhaoshop", self.check_callback)
        producer.set_name_server_address(
            f"{settings.ROCKETMQ_HOST}:{settings.ROCKETMQ_PORT}"
        )
        producer.start()
        msg = Message("order_reback")
        msg.set_keys("zhaoshop")
        msg.set_tags("order")
        ...
        msg_body = {
            ...
            "post": request.post,
            "name": request.name,
            "parent_span": parent_span.context.span_id,
        }
            ...
            if order_sn in local_execute_dict:
                ...
                if local_execute_dict[order_sn]["code"] == grpc.StatusCode.ok:
                    return order_pb2.OrderInfoResponse(
                        ...
                        order_sn=local_execute_dict[order_sn]["order"]["orderSn"],
