...
class OpenTracingClientInterceptor(
    ...
    def __init__(self, tracer, active_span_source, log_payloads, span_decorator):
        self._tracer = tracer
    ...
    def intercept_unary(self, request, metadata, client_info, invoker):
        with self._start_guarded_span(client_info.full_method) as guarded_span:
            metadata = _inject_span_context(self._tracer, guarded_span.span, metadata)
            rpc_info = RpcInfo(
    ...
    def intercept_stream(self, request_or_iterator, metadata, client_info, invoker):
        ...
        with self._start_guarded_span(client_info.full_method) as guarded_span:
            ...
            if self._log_payloads:
                request_or_iterator = log_or_wrap_request_or_iterator(
                    guarded_span.span, client_info.is_client_stream, request_or_iterator
                )
            try:
                result = invoker(request_or_iterator, metadata)
            except:
