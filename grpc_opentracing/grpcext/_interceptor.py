...
class _InterceptorStreamUnaryMultiCallable(grpc.StreamUnaryMultiCallable):
    ...
    def __call__(self, request_iterator, timeout=None, metadata=None, credentials=None):
        def invoker(request_iterator, metadata):
            return self._base_callable(request_iterator, timeout, metadata, credentials)
...
class _UnaryServerInfo(collections.namedtuple("_UnaryServerInfo", ("full_method",))):
    pass


class _StreamServerInfo(
    collections.namedtuple(
        "_StreamServerInfo", ("full_method", "is_client_stream", "is_server_stream")
    )
):
    pass
...
class _InterceptorRpcMethodHandler(grpc.RpcMethodHandler):
    ...
    def unary_unary(self):
        ...
        def adaptation(request, servicer_context):
            ...
            return self._interceptor.intercept_unary(
                request, servicer_context, _UnaryServerInfo(self._method), handler
            )
