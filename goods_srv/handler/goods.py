...
from goods_srv.model.models import GoodsCategoryBrand
...
class GoodsServicer(goods_pb2_grpc.GoodsServicer):
    ...
    def GoodsList(self, request: goods_pb2.GoodsFilterRequest, context):
        ...
        if request.isHot:
            goods = goods.filter(Goods.isHot.contains(request.isHot))
        if request.isNew:
            goods = goods.filter(Goods.isNew.contains(request.isNew))
        if request.priceMax:
        ...
        if request.topCategory:
            ...
            if request.pagePerNums:
                per_page_nums = request.pegePerNums
            if request.pages:
    ...
    def CreateGoods(self, request: goods_pb2.CreateGoodsInfo, context):
        ...
        goods.shop_price = request.shipFree
    ...
    def category_model_to_dict(self, category):
        ...
        re["level"].level = category.level
        re["isTab"].isTab = category.is_tab
        return re
