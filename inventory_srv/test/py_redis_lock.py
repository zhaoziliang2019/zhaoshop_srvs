...
class Lock(object):
    ...
    def __init__(
        ...
        if id is None:
            ...
            raise TypeError(
                f"Incorrect type for `id`. Must be bytes/str not {type(id)}."
            )
    ...
    def acquire(self, blocking=True, timeout=None):
        ...
        if timeout:
            ...
            if (
                self._expire
                and not self._lock_renewal_interval
                and timeout > self._expire
            ):
                raise TimeoutTooLarge(
                    f"Timeout ({timeout}) cannot be greater than expire ({self._expire})"
                )
        ...
        while busy:
            ...
            if busy:
                if timed_out:
                    ...
                    timed_out = (
                        not self._client.blpop(self._signal, blpop_timeout) and timeout
                    )
                else:
                    logger_for_acquire.warning(
                        "Failed to acquire Lock(%r).", self._name
                    )
                    return False
    ...
    def extend(self, expire=None):
        ...
        if error == 1:
            raise NotAcquired(
                f"Lock {self._name} is not acquired or it already expired."
            )
        elif error == 2:
    ...
    def _lock_renewer(name, lockref, interval, stop):
        ...
        while not stop.wait(timeout=interval):
            ...
            lock: "Lock" = lockref()
            if lock is None:
                logger_for_refresh_thread.debug(
    ...
    def _stop_lock_renewer(self):
        ...
        """
        if (
            self._lock_renewal_thread is None
            or not self._lock_renewal_thread.is_alive()
        ):
            return
        logger_for_refresh_shutdown.debug(
            "Signaling renewal thread for Lock(%r) to exit.", self._name
        )
    ...
    def __enter__(self):
        ...
        if not acquired:
            raise AssertionError(
                f"Lock({self._name}) wasn't acquired, but blocking=True was used!"
            )
        return self
